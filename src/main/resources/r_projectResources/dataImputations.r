...
library("RNeo4j", lib.loc="/usr/lib64/R/library")
library("naniar")
library("ggplot2")
library("imputeTS")
library("dplyr")
...
graph <- startGraph("http://192.168.1.94:7474/db/data", username="neo4j", password="george")
...
for (i in 2:length(dt.cols)) {
  try (
    dtma[, i] <- na.interpolation(dtcp[[i]], option = 'spline')
  )
}
...
dt.missing[, years := as.character(year)]
dt.missing$year
dt.missing
